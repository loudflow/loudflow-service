...
class ModelLoader extends LagomApplicationLoader {
  ...
  override def load(context: LagomApplicationContext): LagomApplication = new ModelApplication(context) {
...
abstract class ModelApplication(context: LagomApplicationContext) extends LagomApplication(context) with CassandraPersistenceComponents with LagomKafkaComponents with AhcWSComponents {

  override lazy val lagomServer: LagomServer = serverFor[ModelService](wire[ModelServiceImpl])
  override lazy val jsonSerializerRegistry: JsonSerializerRegistry = ModelSerializerRegistry
